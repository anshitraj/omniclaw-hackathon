...
export async function POST(req: Request) {
  ...
  if (activeRail !== 'demo') {
    try {
      ...
      const receipt = {
        ...
        settlementMetadata: {
          paymentRail: isLegacyDirect ? 'Legacy Direct Mode' : 'Circle Nanopayments / Arc Fallback',
          ...
          policyLayer: 'OmniClaw Financial Control Layer',
          agentKeyAccess: 'none',
          hackathonRequirement: 'sub-cent pricing + 50+ transaction proof',
  ...
  const receipt = {
    ...fallback,
      route: 'Circle Nanopayment Rail (Demo Fallback)',
    settlementMetadata: {
      ...fallback.settlementMetadata,
      paymentRail: 'Circle Nanopayments',
